...
class SatisfactionAnalysis:
  ...
  def satisfaction_analysis(self):
    ...
    aggregation_result = self.df.groupby('MSISDN/Number').agg(aggregate)
    ...
    file_name = 'Telecom_challenge_dataset.csv'
    df_sat1 = pd.read_csv(file_name)
    new_netwok_df = df_sat1[['MSISDN/Number', 'TCP DL Retrans. Vol (Bytes)', 'TCP UL Retrans. Vol (Bytes)',\
    ...
    columns = ['MSISDN/Number','Bearer Id','Total TCP Retrans', 'Total Throughput', 'Total RTT']
    ...
    st.write("In this analysis the satisfuction of the customers towards the services deliverd by the company are analysed .\
       ...
       shows arround 98%. The result shows better but it have not checked using other algorithms.")
